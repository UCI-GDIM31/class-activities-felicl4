...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // STEP 2 -------------------------------------------------------------
        // Write a for loop that visits all of the entries in the _bats array,
        // and for each entry,
        // check the distance between the bat and the player:
        // IF the distance is less than _interactDistance,
        //     make the bat chase the player;
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        TMP_Text reactionText = Instantiate(_reactionUiPrefab, transform);

        reactionText.text = message;

        reactionText.transform.position = bat.transform.position + Vector3.up * 1.5f;

        Destroy(reactionText.gameObject, 1.5f);
